...
class DataflashParser {
    ...
    parseAtOffset (name) {
        ...
        if (parsed.length && this.messageHasInstances(name)) {
            ...
            if (Object.keys(instances).length === 1) {
                ...
                this.postData({ messageType: `${this.fileName}@${name}`, messageList: this.messages[name] })
                return parsed
            }
            for (const [index, messages] of Object.entries(instances)) {
                ...
                this.postData({
                    messageType: `${this.fileName}@${newName}`,
    ...
    fixData (message) {
        ...
        // eslint-disable-next-line
        message.time_boot_ms = (message.TimeUS / 1000) - (this.timeOffset || 0)
    ...
    syncTime (timeOffset) {
        this.timeOffset = parseFloat(timeOffset)
        for (const name of this.alreadyParsed.filter((value, index, array) => array.indexOf(value) === index)) {
            this.parseAtOffset(name)
        }
        self.postMessage({ timeSyncFinished: true })
    }
    ...
    processData (data) {
        ...
        self.postMessage({ availableMessages: messageTypes, fileName: this.fileName })
